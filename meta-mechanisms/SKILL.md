...
| `checks/records-index.sh` | run by an agent before it reads a record | LEDGER.yaml, CORRECTIONS.yaml | One line per item to act on, each keeping its line number, so what an agent must read does not grow with the project; since contract-024 also every live candidate, grouped by target, so whether a claim already exists is one read |
| `checks/G7-batch.sh` | run by the batch assembler before it seals a key, and by the walk | one batch file | Fails when one sitting would put more than six items or 1,200 words to the pioneer, when the preamble is missing or over five sentences, when an item lacks its kind, why or what-is-asked line, or when a read is asked of an item that is not a candidate (contract-024) |
