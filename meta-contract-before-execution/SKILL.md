...
**The mechanisms read these marker keys** — `status`, `verification_state` (`none` · `awaiting-evidence` · `reported` · `closed-by-follow-up` · `legacy`), `audited` (`true` · `false` · `legacy`), `bearing`, `transcript`. `approval` (`approved-at-gate` · `waived-at-spec-lock`; `gate` on entries from before 0.15) is read by no mechanism and recorded for the pioneer. The stop-gate uses them to launch the session auditor and the verifier without anyone asking. Entries that predate kit v0.14 carry `legacy` so history is not re-audited. Analysis reports logged here carry `type: analysis-report` and a `report-NNN` id, and the gates skip them.
