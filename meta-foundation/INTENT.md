...
**Two registers, never collapsed.** `meta-foundation` governs how the work is done. `FOUNDING.md`, loaded beside this file, governs what the work is in service of. Every contract's bearing is read against the founding statement, not against this file.

**The pioneer is named for the journey.** The end result is not known at the start. The journey is exploration, and collaboration between human and agent: the practice guides the pioneer, while the pioneer guides and directs the kit being built.
...
- **Stop on named triggers.** The agent stops and re-orients when a checkable shape appears, regardless of how the work feels — the five shapes are listed in `meta-antidrift/SKILL.md` → Scoring Rules.
...
**The kit stops what a check can stop, and makes the rest visible.** A refused write, a denied read, a failing check — those are stopped by mechanism. What no check can see, the human stops, and the agent accepts the stop without resistance.
...
- **Translate for the pioneer.** What reaches them says what it is, why now and what is asked, beside the record's own words, never instead of them; no id or number stands alone (`meta-foundation`).
