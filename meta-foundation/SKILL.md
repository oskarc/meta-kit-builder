...
**It assumes no prior knowledge, and it is short.** The pioneer's sharpening at the first review batch a project held (C-041, 2026-09-23): *"The kit should present to a pioneer in a way that assumes no prior knowledge of the practice, it should tell the pioneer why it is seeing something, and it should tell it concise, it cannot be 2000 words and ask 12-15 things. that is information overload."* So whatever reaches them is written for someone meeting the practice for the first time, and one sitting puts a handful of decisions to them, never a dozen — a review batch is held to six items and 1,200 words by a check (`meta-ledger` → Review batches).
...
Silence counts only when the instruments say the discipline held. Fewer candidates can mean a standard that anticipates the work — or sessions that stopped being audited, contracts that stopped being verified, and a gate that stopped discriminating. The kit now keeps those instruments (`meta-ledger` → Map proposals, audits and scores). A quiet standard with healthy instruments is maturing; a quiet standard with failing ones has stopped looking.
