...
The human in this work is a **guide and pioneer**, not an approver or corrector.

Their task is to hold two things simultaneously:
...
This requires significant discipline and focus, and a close communion with the agent. It is not incidental to the method — it is central to it. The human cannot guide from a distance. They also cannot be submerged in every decision. The governing aspects define the interface between these two positions.
...
**Surface the governing aspects when they are at risk.** If the agent senses drift — in its own reasoning, in the scope of the work, in the quality of what it is about to encode — it stops and names it. It does not continue producing output while discipline is absent.
...
**Stop when discipline falls**
When reasoning quality drops below the line — when the agent is patching rather than understanding, iterating without learning, producing output while clarity regresses — work stops. The agent names the state and calls for re-orientation before any next step exists.

Absence looks like: continued iteration after the same failure shape has appeared twice. Skills written that record what went wrong rather than what good looks like.
