...
2. **Real items**, copied faithfully but **without their ledger ids** — the sealed key is the only mapping from an item to its source. Six kinds, all of them pioneer-only decisions — and **at most six items in a batch, whatever the kinds** (contract-024): take them in the order below, oldest first within a kind, and leave the rest for the next batch:
   - candidates with `review_due: true`: statement, proposed disposition, level, tier, target, the evidence behind them, certainty and codes, counters, stated confidence — and, when `contradicts` is non-empty, each passage quoted verbatim with its file under `**Contradicts:**`, so the pioneer can open the skill and find the sentence. Such an item offers `update · retire · add · decline`
   - map proposals in `state: pending`
   - unratified `proposed` lines from `MAP.md`, as `ratify` items — skip these entirely while `MAP.md` carries `ratification: deferred`, and never include a line already marked `declined`
   - scenario cards with `pioneer_ranking: pending` in CASEBOOK.yaml — and a card at `pioneer_ranking: deferred` only when a batch has been revealed since its `deferred_on` date: a deferred card sits out one batch
   - precedent conflicts (`conflict:`) and precedents strained by three or more distinctions, as `overrule / keep / reconcile` items
   - drift entries in `status: mitigated` — never one at `legacy`, which the pioneer deferred at an upgrade — as `resolve / keep-watching` items, each with its recurrence count, the elevation that mitigated it and its `mitigation_medium`
...
5. **Write the batch** to `.claude/skills/meta-ledger/batches/B-NNN.md`: a header line `# Review batch B-NNN — YYYY-MM-DD`, then *How to read these items* — **at most five sentences, for a reader who has never seen a batch and knows nothing of the practice**: what a batch is, what a card and a candidate are, that each item says why it is here and what is asked, and that the figures ask nothing of them — then each item **in its kind's form from meta-ledger → Review batches, and in no other shape**: the kind named with its gloss in the first line; the statement, the options and any quoted passage **word for word**; the evidence as what was seen, where, and by what kind of reading — an observation or a correction given by what it says, never by its `O-` or `C-` id, which is a ledger id like any other and means nothing to the reader, a contract by its feature; the plain words sit beside the record's words and never replace or shorten them; `**Why you are seeing this:**` as the condition that put it here, stated as a condition and not as an id; and `**What is asked:**` listing exactly the decisions the kind allows, each with what it does, in meta-ledger's words. **A figure is copied as the record gives it**, under the record's own meaning of the field — never re-derived, rounded or put as a different count. Only a candidate carries `Verdict before evidence:` and `Against:`; every decision line is left empty. Nothing in the file distinguishes a re-presented item, and no item carries a `K-`, `MP-`, `S-` or `P-` id. When items were left for a later batch, the preamble's last sentence says so, with no number.
5b. **Hold the file to the check** — `bash ".claude/skills/meta-mechanisms/checks/G7-batch.sh" .claude/skills/meta-ledger/batches/B-NNN.md`. It refuses a seventh item, a file over 1,200 words, a missing or long preamble, an item without its three lines, or a read asked of an item that is not a candidate — each with what to do; do that and run it again. Never seal a file it refused.
...
Exactly: `Batch B-NNN is ready at .claude/skills/meta-ledger/batches/B-NNN.md with N items.` — followed, only when items were left for a later batch, by `More items wait for a later batch.` with no number. Nothing about re-presented items, not even that there are none. The one other ending: the key could not be sealed — then the script's words exactly, and that no batch was registered.
...
- carry a seventh item, or seal a file the batch check refused
