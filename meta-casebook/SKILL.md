...
Where a correction shows a decision among real options, the clerk drafts a card: the situation at the decision point, and the options as they stood. **The ranking, the rationale and any dissent come only from the pioneer** — cards reach the pioneer as items in a review batch, and a card without a pioneer ranking stays `pending`. A pioneer who does not want to rank a card decides `decline`, which sets `pioneer_ranking: declined` so it is not presented again. A pioneer who does not want to rank it *yet* decides `defer` (contract-024): `pioneer_ranking: deferred` with `deferred_on`, counted by no reader as waiting, and carried again by the assembler once a later batch has been revealed — it sits out one batch. A card carries no evidence and no certainty, so it has no strength to filter on; defer is what stands in for a candidate's hold. A minority view is recorded when the pioneer gives one; it shows that more than one answer was defensible.
