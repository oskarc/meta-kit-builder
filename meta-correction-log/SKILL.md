...
- **An override is not an error.** The log records where the pioneer's judgement was exercised, not where the agent was wrong: presenting a choice with a lean and being overruled is the practice working, and a log in which every entry reads `redirected` is not a log of mistakes. The pioneer's own ranking, at the first batch downstream: the agent's framing above their own override — *"i understand when i want you to push ahead, but you dont. Its my point to hold."* (contract-024)
- **Supersession.** A later correction that reverses an earlier one names it in `supersedes`. Nothing is edited or deleted; like an architecture decision record, the history of a changed mind is part of the record. The case clerk marks the earlier correction's precedent overruled by the later correction — carrying the pioneer's act, not making one (contract-024).
