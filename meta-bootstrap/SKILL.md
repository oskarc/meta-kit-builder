...
**The install sheet** (Step 2) carries, in this order: proceed with the practice as introduced; is the orientation correct; the applications in the system flow; the library kit found, integrate or not; the CLAUDE.md block, shown, write or not; the settings merge, shown as its diff, merge or not; ratify the base map now or later. The founding statement is not on it — it has its own moment (Step 3).
