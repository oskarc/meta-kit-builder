...
**The agent holds five.** *Lay of the land*, *Stop on named triggers*, *Partner as orientation mirror*, *Evolution from elevation*, *Evidence is the work*. These are checkable disciplines: the session auditor scores the agent on them from outside the session, and the agent no longer scores itself — it closes every output by answering four questions about what its framing rests on, which are the pioneer's to read.
...
7. **Review batch.** When items only the pioneer can decide are waiting, the batch assembler builds a batch from real items. From the fourth batch on it may include up to two items the pioneer already decided, shown again as if new. While the batch is open the ledger is closed to the session presenting it, which is what keeps those items indistinguishable; the casebook stays open. Every item is written for a reader who has not seen the ledger: what it is in plain words, why it is in front of them now, and what is asked with what each answer does, beside the record's own words. The pioneer reads each statement, answers one question — *assume this is wrong: why?* — before seeing the evidence, then decides.
...
| `seal-key.sh` | run by the batch assembler | writes a batch's sealed key — the program underneath refuses the file tools in that folder, so the key is sealed and opened by scripts; refuses a key that does not name every item, and never replaces or prints one |
| `agent-launch.sh` | PreToolUse, when an agent is launched | records that one is starting, with one number standing for the content of everything a kit agent may write, so the gate can tell a running agent from one that exhausted in silence — and it reads only the kit's own agents, not a project's other helpers |
| `mark-done.sh` | run by the agent at the end of an install or upgrade | marks the end in telemetry; the hold on the first review batch comes from the baseline the run wrote, so it holds even if this is forgotten |
...
| `checks/G6-refusals.sh` | run by the verifier and on upgrade | fails when a refusal the kit can print names no way forward — every one is registered with its next step; a project registers its own in `refusal-nextsteps.project.txt`, which no upgrade replaces |
| `checks/waiting-on-you.sh` | read by the agent at the start of a sitting | lists what is queued on the pioneer's decision, oldest first — a review batch as one line with no count |
| `checks/records-index.sh` | run by an agent before it reads a record | one line per item to act on, each keeping its line number, so what an agent must read does not grow with the project |
| `checks/G7-batch.sh` | run by the batch assembler before sealing, and by the walk | fails when one sitting would put more than six items or 1,200 words to the pioneer, when the preamble is missing or long, or when an item lacks its kind, why or what-is-asked line (contract-024) |
| `checks/transcript-digest.sh` | run by the agent before the session auditor | turns a session transcript into one row per turn, so the audit can start whatever the transcript's size; prints the path in the spelling the auditor's file tools can open |
...
meta-understanding/     SKILL.md — the four questions every output ends with
...
docs/                   kit-flow.html (the flow for a newcomer) · ROADMAP.md (what is coming, and why) ·
                        readme-review.md · rebuild-design.md · reports/
...
Never copy a new kit over `.claude/skills/`. It would overwrite your manifest, logs, ledger and corrections, and flatten any skill your project has evolved. Instead, build a release in the kit's repository — `bash meta-bootstrap/release.sh <folder>` exports exactly the files that ship, with a list of them and their checksums — place that folder in `.claude/kit-incoming/`, and open a session; the session-start hook names the upgrade, and the agent follows the *staged* kit's `meta-bootstrap` — the copy that knows what it added. It is upgraded the way software is: the ground is checked first, and the check refuses, each time with its reason, if the staged kit is not whole, a review batch is open, or you have uncommitted work — it asks you to commit and push first, because the upgrade uses git: your last commit is its snapshot and its way back. If a run stops halfway, one command restores the project exactly and the run is started again, never continued. The upgrade is rehearsed on a copy of your `.claude/` first, silently, with a log of three numbers: decisions asked, files that genuinely differed, and troubleshooting steps. The rehearsal produces one sheet: every question the real run would meet, each with the rehearsal's answer as its default and what each choice changes. A kit skill is asked about only where you wrote lines of your own in it — the upgrade measures them against the copy the kit last installed, takes a re-wrapped line for the kit's, and shows you exactly those lines: reapply them on top of the upgraded skill, or replace. Your own lines are put onto the new skill by git's three-way merge, and where your edit and the kit's change meet in the same lines, the sheet shows both versions and asks which stands. Beside those come at most seven standing questions, each only when it applies: the files the kit no longer ships, which older contracts you want verified, drift entries awaiting review, base lines you changed in a record, the applications your system spans, which project skills belong in the always-loaded map, and whether to ratify the map now or later. You answer once; the real run takes every answer from the sheet, asks nothing, and ends on the same done block as an install. Existing records are never replaced; new fields are added with a default that says they were not there before, and a migration check fails the run if a record came out incomplete. A project from before the map and the hooks existed is upgraded by the same steps, measured against the line history the kit ships.
...
The base building kit is at **v0.32**. Since the six-layer form was built and reviewed (contracts 001 and 002), twenty-one contracts have moved it:
...
- **Contract-014** is the pioneer's presentation rule: whatever reaches them says what it is, why now, and what is asked — no id, code or number stands alone. It is one always-loaded line and one section of the foundation skill, applied to the review-batch item, the summary after a build, how a contract cites a past ruling, and anything an agent's result is passed on as. The plain words sit beside the record's own words and never replace them, and the ask is exactly the one the record allows — the pioneer's pre-mortem was a translation that loses accuracy. The session auditor checks the agent's messages from outside the session.
...
- **Contract-019** came from a day downstream in which a gate asked an agent to do an impossible thing twenty-one times and the agent refused, correctly, twenty-one times — because nothing in the kit could record *this cannot be done*. `audited` was `true` or `false`, so a task nobody could do and a task nobody had got to read the same, and everything below it in the queue went unserved while the pioneer ran the lifecycle by hand. The four lifecycle states now take `blocked` with a reason and what it waits for, on the record beside the state; the gate skips a blocked task, puts it to the pioneer once a sitting with a suggested action, and treats the same hand-over three turns running as a fault rather than a backlog; a documented refusal stops counting as drift; the session auditor reads a digest of one row per turn instead of a transcript whose every line is a whole turn; and every refusal the kit can print is registered with the next step it gives the reader, held by `checks/G6-refusals.sh`.
- **Contract-020** replaced the block that closed every output. It graded the agent on five qualities it could not see the absence of, so it read cleanest exactly when it was most wrong — a downstream report watched a whole working day pass with every gate green over understanding that would not survive one question. Every output to the pioneer now ends with four questions instead, in the agent's own prose, two to three sentences each: what the framing should have rested on, what it did rest on, why that, and how it was put to them. Nothing scores, counts or checks the answers — the pioneer reads them, and a story that is not true stops matching the work within a few rounds. The agent no longer scores itself; `kit-session-auditor` scores the five aspects from the transcript and picks up the approvals acted on and the deviations nobody authorised. A new node, `meta-understanding`, carries why the questions exist.
- **Contract-021** came from the first complete run of the lifecycle in a real project. It ran end to end and the pioneer held it together by hand: four kit agents ran out of turns eight times in two days, every one with the work done and nothing written, and a person restarted each; four times the gate dispatched a second agent while the first still held the ledger; twice it called a task that was waiting on the pioneer a fault. An agent is now recorded starting as well as stopping, with the length of every record it may write, so the gate can tell a running agent from one that exhausted in silence. While an agent runs nothing else is dispatched, and that hold expires like a lease if it never reports finishing. An agent that wrote nothing is resumed once and then recorded blocked. Waiting is no longer called failing. Every hand-over names how many tasks stand behind it. And `checks/records-index.sh` gives an agent one line per item it must act on, each keeping its line number, because what exhausted them was reading a record that grows with every contract.
- **Contract-022** came from drawing the loop and marking every point where it stops and asks the pioneer something. Three of those stops were carrying more than one kind of question: a direction to judge, tests to disappoint, and design choices to pick between, all in the message where getting it wrong costs most. The bearing is now served alone and first, marked as the thing everything else derives from, so rejecting a direction costs two sentences of reading instead of four tiers — and the gate is still one stop. Design choices leave the gate for the spec lock, before the tiers exist; anything not load-bearing the agent decides with its reasoning shown. And `checks/waiting-on-you.sh` lists what is queued on the pioneer, which the session-start hook hands to the agent to put to them at the start of a sitting, so a backlog never again reaches them only because they asked. A review batch appears as one line with no count, because knowing how many items are really due would let the re-presented ones be counted out.
- **Contract-023** came from reading the whole kit for places where one part contradicts another, after a downstream report argued that the sealed key had survived four releases because it lives in the one layer no shell test can reach. There were nineteen, and nine had been introduced by the four contracts before this one — each of which passed its own tests, because each test asked whether the new rule did what it said and none asked what else passes through the point that was changed. The pioneer's ruling on the list: obvious conflicts and errors are handled, not parked. So the key is sealed by a script, as it is opened by one, and a review batch can be assembled for the first time; the batch step is the agent's move and a jam there is called a fault instead of "waiting on you"; every gate task that can block has somewhere to write it; only the kit's own agents are watched, and "wrote nothing" is judged by content; whether a review is due is one rule that three readers call; the auditor is handed a path it can open; and eight sentences that said what the kit no longer does were corrected, their old wordings listed as retired. Every repair is tested from the side it was hidden from — each agent's own procedure run through its own guards, the fingerprint held to the agents' write scopes, the inventory and the self-test held to the folders they describe.
- **Contract-024** came from the first review batch a project ever held — twenty items, five corrections drawn by the review itself. A batch is now held to six items and 1,200 words by a check, every kind of item has a form written for a reader new to the practice, a card can be deferred, the two opening questions are asked of candidates only, the records index lists live candidates by target, a changed mind reaches its precedent, and the correction log says an override is not an error.
