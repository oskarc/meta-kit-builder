...
| `agent-launch.sh` | PreToolUse, when an agent is launched | records that one is starting and how long every record is, so the gate can tell a running agent from one that exhausted in silence |
...
| `checks/G6-refusals.sh` | run by the verifier and on upgrade | fails when a refusal the kit can print names no way forward — every one is registered with its next step |
...
| `checks/transcript-digest.sh` | run by the agent before the session auditor | turns a session transcript into one row per turn, so the audit can start whatever the transcript's size |
...
The base building kit is at **v0.30**. Since the six-layer form was built and reviewed (contracts 001 and 002), twenty contracts have moved it:
