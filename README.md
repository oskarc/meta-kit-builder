...
The base building kit is at **v0.20**. Since the six-layer form was built and reviewed (contracts 001 and 002), ten contracts have moved it:
...
- **Contract-012** drafts Tier 4 with the tiers: the acceptance tests arrive with the bearing and the three tiers, drawn from the agent's picture of the land after implementation so its reading shows before any code; the pioneer's disappointment line per guarantee is still asked and still wins, the tests it moves are realigned before approval and named on a `realigned:` line, and the gate is one stop.
